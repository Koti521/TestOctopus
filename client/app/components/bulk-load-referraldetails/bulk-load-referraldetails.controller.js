class BulkLoadReferralDetailsController {
    ...
    constructor(ExtendedFileUploader, envConfig, appConfig, bulkErrorLogService, fileService) {
        ...
        this.uploader = new ExtendedFileUploader({
            url: envConfig.apiBaseUrl + appConfig.referralDetailsBulkLoadPath,
            removeAfterUpload: true
        });

        this.logUrl = envConfig.apiBaseUrl + appConfig.referralDetailsBulkLoadLogPath;
    }
    ...
    saveFile(logId) {
        let promise = this._bulkErrorLogService.getBulkErrorLogFile('referraldetails', logId).then(result => {
...
export default BulkLoadReferralDetailsController;