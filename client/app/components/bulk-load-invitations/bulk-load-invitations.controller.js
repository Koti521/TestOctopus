class BulkLoadInvitationsController {
    ...
    constructor(
        ...
        this.uploader = new ExtendedFileUploader({
            url: envConfig.apiBaseUrl + appConfig.invitationBulkLoadPath,
            removeAfterUpload: true
        });

        this.logUrl = envConfig.apiBaseUrl + appConfig.invitationBulkLoadLogPath;
    }
        ...
        this.uploader.onCompleteItem = (fileItem, response, status) => {
            ...
            if (status === 200) {
                ...
                this._eventTraceService.trace(
                    'Bulk invitation sending', angular.toJson(fileItem.file), angular.toJson(response), status, fileItem.url);
            } else {
    ...
    saveFile(logId) {
        let promise = this._bulkErrorLogService.getBulkErrorLogFile('invitation', logId).then(result => {
...
export default BulkLoadInvitationsController;