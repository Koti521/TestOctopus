class BulkLoadRemoveController { 
    ...
    constructor(ExtendedFileUploader, envConfig, appConfig, bulkErrorLogService, fileService) {
        ...
        this.uploader = new ExtendedFileUploader({
            url: envConfig.apiBaseUrl + appConfig.removeBulkLoadPath,
            removeAfterUpload: true
        });

        this.logUrl = envConfig.apiBaseUrl + appConfig.removeBulkLoadLogPath;
    }
    ...
    saveFile(logId) {
        let promise = this._bulkErrorLogService.getBulkErrorLogFile('removalexclusion', logId).then(result => {
...
export default BulkLoadRemoveController;