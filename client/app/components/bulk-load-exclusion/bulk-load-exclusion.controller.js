class BulkLoadExclusionController { 
    ...
    constructor(ExtendedFileUploader, envConfig, appConfig, bulkErrorLogService, fileService) {
        ...
        this.uploader = new ExtendedFileUploader({
            url: envConfig.apiBaseUrl + appConfig.exclusionBulkLoadPath,
            removeAfterUpload: true
        });

        this.logUrl = envConfig.apiBaseUrl + appConfig.exclusionBulkLoadLogPath;
    }
    ...
    saveFile(logId) {
        let promise = this._bulkErrorLogService.getBulkErrorLogFile('exclusion', logId).then(result => {
...
export default BulkLoadExclusionController;